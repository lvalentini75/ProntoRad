-- Migration: Fix failed migration 20260505_185035 and create missing debug function
-- The previous migration succeeded in creating RLS policies but failed during verification block
-- This migration only creates the diagnostic function that was missing

-- ============================================================================
-- CREATE DIAGNOSTIC FUNCTION (only thing missing from previous migration)
-- ============================================================================
...
COMMENT ON FUNCTION public.debug_user_can_insert_slot IS 'Debug function to check if current user can insert slots for an organization. Usage: SELECT * FROM debug_user_can_insert_slot(''org-uuid-here'');';
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION '❌ Function debug_user_can_insert_slot not found';
  END IF;
END $$;
