...
class FacilityListScreen extends StatefulWidget {
  ...
  const FacilityListScreen({
    ...
    this.userLon,
            ...
            'Either exam or examPackage must be provided');
...
class _FacilityListScreenState extends State<FacilityListScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      debugPrint(
          '[FacilityList] 📚 Caricati ${_examIdToCategory.length} esami per mappa categoria');
    } catch (e) {
  ...
  Future<void> _loadOrganizations() async {
    ...
    try {
      ...
      debugPrint(
          '   - Exam: ${widget.exam?.name ?? "N/A"} (id=$examId, category=$examCategory)');
      debugPrint(
          '   - Package: ${widget.examPackage?.name ?? "N/A"} (id=$packageId)');
      debugPrint(
          '   - Location: country=${widget.country}, region=${widget.region}, province=${widget.province ?? "ALL"}, city=${widget.city ?? "ALL"}');
      ...
      debugPrint(
          '[FacilityList] 📍 Found ${orgs.length} organizations matching criteria');
      ...
      for (final org in orgs) {
        ...
        try {
          ...
          final slots =
              await _availabilityService.getAllSlotsForOrganization(org.id);

          // Determina la categoria per il match
          final examCategoryUpper =
              widget.exam?.category.name.toUpperCase() ?? '';
          ...
          bool slotMatchesExamOrPackage(AvailabilitySlot s) {
            ...
            if (widget.exam != null) {
              ...
              if (s.examCategory != null &&
                  s.examCategory!.toUpperCase() == examCategoryUpper) {
                return true;
              }
              ...
              if (s.examCategory == null &&
                  s.examId != null &&
                  _examIdToCategory.containsKey(s.examId)) {
                final derivedCategory =
                    _examIdToCategory[s.examId]!.toUpperCase();
          ...
          debugPrint(
              '[FacilityList] Org ${org.name}: ${availableSlots.length} slot disponibili');
          ...
          if (availableSlots.isNotEmpty) {
            ...
            debugPrint(
                '[FacilityList] Org ${org.name}: primo slot disponibile ${firstSlot.startTime}');
          } else {
            debugPrint(
                '[FacilityList] Org ${org.name}: nessuno slot disponibile');
          }
        } catch (e) {
          debugPrint(
              '[FacilityList] Error loading slots for org ${org.id}: $e');
        }
  ...
  void _sortOrganizations(List<OrganizationWithOffering> list) {
    switch (_selectedFilter) {
      ...
      case 'Data/Ora':
        list.sort((a, b) {
          if (a.firstAvailableDate == null && b.firstAvailableDate == null)
            return 0;
          if (a.firstAvailableDate == null) return 1;
          if (b.firstAvailableDate == null) return -1;
          final dateCompare =
              a.firstAvailableDate!.compareTo(b.firstAvailableDate!);
          if (dateCompare != 0) return dateCompare;
          // Confronta gli orari usando startTime (DateTime)
          if (a.firstAvailableSlot == null && b.firstAvailableSlot == null)
            return 0;
          if (a.firstAvailableSlot == null) return 1;
          if (b.firstAvailableSlot == null) return -1;
          return a.firstAvailableSlot!.startTime
              .compareTo(b.firstAvailableSlot!.startTime);
        });
        break;
      case 'Prezzo':
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              icon: Icon(Icons.tune, color: LightModeColors.lightPrimary),
              onPressed: () {}),
        ],
      ),
      body: Column(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${data.distanceKm != null ? '${data.distanceKm!.toStringAsFixed(1)} km • ' : ''}${org.city}${org.province.isNotEmpty ? ', ${org.province}' : ''}',
                        style: TextStyle(
...
class _TimeSlotSheetState extends State<TimeSlotSheet> {
  ...
  // Cache degli esami per derivare la categoria dagli slot con solo examId
  Map<String, String> _examIdToCategory = {};
  // Cache per nomi esami
  Map<String, ExamType> _examCache = {};
  
  // Prerequisiti obbligatori
  List<ExamPrerequisite> _mandatoryPrerequisites = [];
  // Slot prerequisito trovato (per l'esame principale selezionato)
  ...
  /// Carica tutti gli esami per creare una mappa examId -> category e cache esami
  Future<void> _loadExamCategories() async {
    try {
      final exams = await _examService.getAllExams();
      final map = <String, String>{};
      final cache = <String, ExamType>{};
      for (final exam in exams) {
      ...
      // Carica prerequisiti per l'esame corrente
      if (widget.exam != null) {
  ...
  /// Carica i prerequisiti obbligatori per l'esame corrente
  Future<void> _loadPrerequisites() async {
    if (widget.exam == null) return;
    ...
    try {
      ...
      if (mandatory.isNotEmpty) {
        debugPrint('[TimeSlotSheet] ⚠️ Esame ${widget.exam!.name} ha ${mandatory.length} prerequisiti obbligatori');
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                ...
                if (_loadingSlots)
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 12),
                        Text('Nessuno slot disponibile per questa data',
                  ...
                  Wrap(
                    ...
                    children: _availableSlots.map((slot) {
                      ...
                      final remaining = slot.isAvailable
                          ? 1
                          : 0; // TODO: aggiornare con logica corretta
                      return GestureDetector(
                        onTap: remaining > 0
                            ? () async {
                                setState(() => _selectedSlot = slot);
                                // Verifica prerequisiti se presenti
                                if (_mandatoryPrerequisites.isNotEmpty) {
                                  await _checkPrerequisiteSlot(slot);
                                }
                              }
                            : null,
                        child: Container(
              ...
              // Mostra prerequisiti obbligatori se presenti
              if (_mandatoryPrerequisites.isNotEmpty && _selectedSlot != null) ...[
                const SizedBox(height: 24),
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: _prerequisiteSlot != null 
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Esame con Prerequisito Obbligatorio',
                              style: TextStyle(
                      ...
                      const SizedBox(height: 12),
                      Text(
                        'L\'esame "${widget.displayName}" richiede che venga eseguito prima:',
                        style: TextStyle(
                      ...
                      const SizedBox(height: 8),
                      ...(_mandatoryPrerequisites.map((prereq) {
                        final prereqExam = _examCache[prereq.prerequisiteExamId];
                        final prereqTime = _selectedSlot!.startTime.subtract(
                          Duration(minutes: prereq.timeGapMinutes),
                        );
                        final timeStr = '${prereqTime.hour.toString().padLeft(2, '0')}:${prereqTime.minute.toString().padLeft(2, '0')}';
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  '${prereqExam?.name ?? "Esame prerequisito"} alle $timeStr',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.w600,
                                    fontSize: 14,
                                  ),
                      ...
                      if (_prerequisiteSlot != null) ...[
                        const SizedBox(height: 8),
                        Text(
                          '✓ Slot disponibile trovato - verranno create 2 prenotazioni',
                          style: TextStyle(
                        ...
                        const SizedBox(height: 8),
                        Text(
                          '⚠️ Nessuno slot disponibile per il prerequisito a quest\'ora. '
                          'Seleziona un altro orario o contatta la struttura.',
                          style: TextStyle(
              ...
              if (_selectedSlot != null && _prerequisiteSlot != null && _prerequisiteExam != null) ...[
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildBookingSummaryRow(
                        '1. ${_prerequisiteExam!.name}',
                        _prerequisiteSlot!.startTimeFormatted,
                      ),
                      const SizedBox(height: 8),
                      _buildBookingSummaryRow(
                        '2. ${widget.displayName}',
                        _selectedSlot!.startTimeFormatted,
                      ),
                    ],
                  ),
                ),
              ...
              ElevatedButton(
                    ...
                    ? () {
                        ...
                        context.push(
                          ...
                          extra: {
                            ...
                            'price': price,
                            // Passa dati prerequisito se presente
                            if (_prerequisiteSlot != null && _prerequisiteExam != null) ...{
                              'prerequisiteExam': _prerequisiteExam,
                              'prerequisiteSlot': _prerequisiteSlot,
                              'prerequisiteTime': _prerequisiteSlot!.startTime,
                            },
                          },
                        );
                      }
                ...
                child: Text(_mandatoryPrerequisites.isNotEmpty && _prerequisiteSlot != null
  ...
  /// Verifica se esiste uno slot prerequisito disponibile per lo slot selezionato
  Future<void> _checkPrerequisiteSlot(AvailabilitySlot mainSlot) async {
    if (_mandatoryPrerequisites.isEmpty) {
      setState(() {
        _prerequisiteSlot = null;
        _prerequisiteExam = null;
      });
      return;
    }
    
    // Per semplicità gestiamo solo il primo prerequisito obbligatorio
    final prereq = _mandatoryPrerequisites.first;
    final prereqExamId = prereq.prerequisiteExamId;
    final timeGap = prereq.timeGapMinutes;
    
    // Calcola l'orario richiesto per il prerequisito (es: 30 min prima)
    final prereqTime = mainSlot.startTime.subtract(Duration(minutes: timeGap));
    
    debugPrint('[TimeSlotSheet] 🔍 Cercando slot prerequisito:');
    debugPrint('   - Esame prerequisito: $prereqExamId');
    debugPrint('   - Orario richiesto: ${prereqTime.hour}:${prereqTime.minute.toString().padLeft(2, '0')}');
    debugPrint('   - Data: ${prereqTime.day}/${prereqTime.month}/${prereqTime.year}');
    
    // Cerca uno slot disponibile per l'esame prerequisito alla data/ora calcolata
    try {
      final allSlots = await _availabilityService.getAllSlotsForOrganization(widget.organization.id);
      
      final prereqExamCategory = _examIdToCategory[prereqExamId]?.toUpperCase() ?? '';
      
      // Filtra slot per l'esame prerequisito
      final prereqSlots = allSlots.where((s) {
        // Match per examId
        if (s.examId == prereqExamId) return true;
        // Match per categoria
        if (s.examCategory != null && s.examCategory!.toUpperCase() == prereqExamCategory) return true;
        // Fallback: deriva categoria da examId
        if (s.examId != null && _examIdToCategory.containsKey(s.examId)) {
          final derived = _examIdToCategory[s.examId]!.toUpperCase();
          if (derived == prereqExamCategory) return true;
        }
        return false;
      }).where((s) => s.isAvailable).toList();
      
      debugPrint('[TimeSlotSheet] 📊 Slot disponibili per prerequisito: ${prereqSlots.length}');
      
      // Cerca uno slot che inizia all'orario calcolato (con tolleranza di 10 minuti)
      AvailabilitySlot? foundSlot;
      for (final slot in prereqSlots) {
        final sameDay = slot.startTime.year == prereqTime.year &&
            slot.startTime.month == prereqTime.month &&
            slot.startTime.day == prereqTime.day;
        
        if (!sameDay) continue;
        
        final slotMinutes = slot.startTime.hour * 60 + slot.startTime.minute;
        final reqMinutes = prereqTime.hour * 60 + prereqTime.minute;
        final diff = (slotMinutes - reqMinutes).abs();
        
        if (diff <= 10) {
          foundSlot = slot;
          debugPrint('[TimeSlotSheet] ✅ Trovato slot prerequisito: ${slot.startTimeFormatted}');
          break;
        }
      }
      
      if (foundSlot == null) {
        debugPrint('[TimeSlotSheet] ⚠️ Nessuno slot prerequisito trovato all\'orario richiesto');
      }
      
      if (mounted) {
        setState(() {
          _prerequisiteSlot = foundSlot;
          _prerequisiteExam = _examCache[prereqExamId];
        });
      }
    } catch (e) {
      debugPrint('[TimeSlotSheet] ❌ Errore verifica prerequisito: $e');
    }
  }
  ...
  Future<void> _onDateSelected(DateTime date) async {
    ...
    try {
      ...
      debugPrint(
          '[TimeSlotSheet] 🔍 Carico slot per org=${widget.organization.id} (${widget.organization.name}), exam=$examName, date=${date.day}/${date.month}/${date.year}');

      // Carica tutti gli slot per questa organizzazione
      final allSlots = await _availabilityService
          .getAllSlotsForOrganization(widget.organization.id);

      debugPrint(
          '[TimeSlotSheet] 📥 Ricevuti ${allSlots.length} slot totali dal DB');
      ...
      bool slotMatchesExamOrPackage(AvailabilitySlot s) {
        ...
        if (widget.examPackage != null && packageExamIds.isNotEmpty) {
          if (s.examId != null && packageExamIds.contains(s.examId))
            return true;
        }
      ...
      debugPrint(
          '[TimeSlotSheet] 🔍 Slot per $examName: ${slotsForExam.length}');

      final availableSlots = slotsForExam.where((s) => s.isAvailable).toList();
      debugPrint(
          '[TimeSlotSheet] 🔍 Slot disponibili (is_available=true): ${availableSlots.length}');
      ...
      debugPrint(
          '[TimeSlotSheet] ✅ Slot finali per ${date.day}/${date.month}: ${slots.length}');
      if (slots.isEmpty && slotsForExam.isNotEmpty) {
        debugPrint(
            '[TimeSlotSheet] ℹ️ Gli slot per questo esame esistono ma non per la data ${date.day}/${date.month}');
        // Log delle date disponibili
        final dates = slotsForExam
            .map((s) => '${s.startTime.day}/${s.startTime.month}')
            .toSet();
        debugPrint(
            '[TimeSlotSheet] 📅 Date con slot disponibili: ${dates.join(", ")}');
      }
  ...
  bool _canConfirmBooking() {
    ...
    // Se ci sono prerequisiti obbligatori, deve esserci lo slot prerequisito
    if (_mandatoryPrerequisites.isNotEmpty) {
      return _prerequisiteSlot != null && _prerequisiteExam != null;
    }
    
    return true;
  }
  
  /// Widget per mostrare una riga del riepilogo prenotazioni
  Widget _buildBookingSummaryRow(String examName, String time) {
    return Row(
      children: [
        Icon(Icons.access_time, size: 16, color: LightModeColors.lightPrimary),
        const SizedBox(width: 8),
        Expanded(
          child: Text(
            examName,
